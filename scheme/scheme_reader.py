...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 2
            "*** YOUR CODE HERE ***"
            src.remove_front()  # remove this dot
            # dot 后面的东西应该在一个()里面，怎么判断这一点呢？
            if src.current() == '(':
                first_element_after_dot = scheme_read(src)  # 如果是legal dotted pair, 那么此时余下的src应该只剩下')'了吧？
                if src.current() == ')':
                    return first_element_after_dot
                else:
                    raise SyntaxError('ill-formed dotted pairs ')  # 之前忘记加上这里的else了，加了之后就猝不及防地pass了。
            element = src.remove_front()
            if src.remove_front() == ')':
                return element
            raise SyntaxError('ill-formed dotted pairs ')
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
...
def buffer_lines(lines, prompt='scm> ', show_prompt=False):
    ...
    if show_prompt:
        input_lines = lines
    else:       input_lines = LineReader(lines, prompt)
...
def read_print_loop():
    ...
    while True:
        try:
            ...
            print()
            return
Pair('+', Pair(1, Pair(Pair('+', Pair(23, Pair(4, nil))), nil)))
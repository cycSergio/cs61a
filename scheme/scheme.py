...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args = rest.map(lambda t: scheme_eval(t, env))
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        while copy.parent is not None:
            if symbol in copy.parent.bindings:
                return copy.parent.bindings[symbol]
            else:
                copy = copy.parent
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        body_here = LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first, body_here)
        return target.first
        # END PROBLEM 10
    else:

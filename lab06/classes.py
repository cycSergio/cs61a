...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        if type(person) != str:
            ...
            if person in where_to_find:
                m = where_to_find[person].talk()  # 原来talk后面要加上()!怎么之前ants里写了那么多都没感觉？
                print(person, 'says:', m)
            else:
    ...
    def take(self, thing):
        ...
        if type(thing) != str:
            ...
            thing_place = self.place.things # this is a dictionary哦
    ...
    def unlock(self, place):
        ...
        for item in self.backpack:
            if type(item) == Key:
                key = item
        "*** YOUR CODE HERE ***"
        if key and self.unlock_time == 0:
            the_place_to_change = self.place.place_dict[place]
            key.use(the_place_to_change)
            print(place, 'is now unlocked!')
            self.unlock_time += 1
        elif key and self.unlock_time > 0:
...
""" Implement Key here! """
class Key(Thing):
    def use(self, place):  # 这里的place是一个object哈
        place.locked = False
...
class Place(object):
    ...
    def __init__(self, name, description, characters, things):
        ...
        self.locked = True  # 是把False改成True吗？
        self.exits = {} # {'name': (exit, 'description')}
        self.place_dict[self.name] = self

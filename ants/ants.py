...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        exit -- The Place reached by exiting this Place (may be None). ？？？啥意思啊
        ...
        # Phase 1: Add an entrance to the exit 啥意思，没懂。给出口添一个入口？？？？？？
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        if self.exit != None:
            exit.entrance = self
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                elif insect.can_contain(self.ant):  # 为什么改成if就会出现'ThrowerAnt' object has no attribute 'can_cantain'
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and insect.if_queen:  # 判断这个insect是不是queen的instance
                return
            # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is None:
            return False
        elif self.place.ant is not None:
...
class HarvesterAnt(Ant):
    ...
    name = 'Harvester'
    implemented = True
    food_cost = 2  # 我根据Q1的指示加的，不知道对不对啊
...
class ThrowerAnt(Ant):
    ...
    implemented = True
    damage = 1
    food_cost = 3  # 我根据Q1的指示加的，不知道对不对啊
    min_range = 0  # Q4 这个min_range的attribute应该加在这里还是加在nearest_bee方法里面啊？？？
    max_range = float('inf')  # 无穷大可以这样表示，感谢题目的Hint，哈哈
    ...
    def nearest_bee(self, hive):
        ...
        啊，题目还有一点要求是：Don't change self.place!!
        """
        # BEGIN Problem 3 and 4
        # 下面的写法违反了不能change self.place的要求哦。
        # while self.place.bees == []:
        #     if self.place.entrance == Hive:
        #         return None
        ...
        #         range += 1
        # if self.min_range > 0:
        #     while curr_place.bees == [] or range < self.min_range: # 这里要用or诶，第一次写的时候竟然写的是and
        #         if curr_place.entrance.name == 'Hive':
        #             return None
        #         curr_place = curr_place.entrance
        #         range += 1
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        "*** YOUR CODE HERE ***"
        # if insect.watersafe:
        #    insect.place.add_insect(insect) 这是一个没有解决的疑问，为什么这样写不行，非得写成下面那句？？
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        Ninja_copy_bees = self.place.bees[:]
        for bee in Ninja_copy_bees:
            Insect.reduce_armor(bee, self.damage)
        # END Problem 7
...
class HungryAnt(Ant):
    ...
    def __init__(self):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        self.digesting = 0  # 第一次写时忘记了前面的self！为什么要加上self？去看笔记里的attribute assignment哈
        Ant.__init__(self)
        # END Problem 6
    ...
    def action(self, colony):
        ...
        if self.digesting == 0 and self.place.bees != []:
            return self.eat_bee(random_or_none(self.place.bees))  # 这里要传入几个参数?区分function和method！
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        Tank_bees_copy = self.place.bees[:]
        for bee in Tank_bees_copy:
            Insect.reduce_armor(bee, self.damage)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    food_cost = 7
    instance_sum = 0
    ants_been_doubled = []  # 是要写在class attribute里的吧？第一次写的时候写在method里面，感觉不太对
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        Ant.__init__(self)
        QueenAnt.instance_sum += 1  # 这里dot前面不能是self，而应该是QueenAnt！还是attribute assignment的内容哦。
        self.id = self.instance_sum
        self.if_queen = False
        if self.id == 1:
            self.if_queen = True
        # END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if not self.if_queen:
            return self.reduce_armor(self.armor)
        ThrowerAnt.action(self, colony)
        back_location = self.place.exit  # 当前ant所在place的后一个，也就是其出口
        # while back_location:  # 当back_location is not None
        #     if back_location.ant is not None and back_location.ant not in self.ants_been_doubled:
        #         back_location.ant.damage = back_location.ant.damage * 2
        #         self.ants_been_doubled.append(back_location.ant)
        #         # 如果一个place里面有2个蚂蚁的话,???
        #         # 如果先有了container：
        #         if back_location.ant.container and back_location.ant.ant is not None:
        #             back_location.ant.ant.damage = back_location.ant.ant.damage * 2
        #             self.ants_been_doubled.append(back_location.ant.ant)
        #         # 如果先有了被contain的：
        #         if back_location.ant.container and back_location.ant.ant in self.ants_been_doubled:
        #             back_location.ant.damage = back_location.ant.damage * 2
        #             self.ants_been_doubled.append(back_location.ant)
        #     back_location = back_location.exit
        while back_location:
            if isinstance(back_location.ant, BodyguardAnt) or isinstance(back_location.ant, TankAnt):
                if back_location.ant not in self.ants_been_doubled:
                    back_location.ant.damage = back_location.ant.damage * 2
                    self.ants_been_doubled.append(back_location.ant)
                if back_location.ant.ant and back_location.ant.ant not in self.ants_been_doubled:
                    back_location.ant.ant.damage = back_location.ant.ant.damage * 2
                    self.ants_been_doubled.append(back_location.ant.ant)
            elif back_location.ant is not None and back_location.ant not in self.ants_been_doubled:
                back_location.ant.damage = back_location.ant.damage * 2
                self.ants_been_doubled.append(back_location.ant)
            back_location = back_location.exit
...
def make_slow(action):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    # 自己卡住的地方在于，colony.time从哪里来？→lambda哈 & action后要不要传入colony
